...
> 🐳 **没有 OpenClaw？** 跑一行 `docker run -p 7892:7892 cft0808/edict` 即可体验完整看板 Demo（预置模拟数据）。
...
```bash
docker run -p 7892:7892 cft0808/sansheng-demo
```
打开 http://localhost:7892 即可体验军机处看板。
...
```bash
docker run --platform linux/amd64 -p 7892:7892 cft0808/sansheng-demo
```
...
```bash
# 添加远程 skill
curl -X POST http://localhost:7892/api/add-remote-skill \
...
# 查看所有远程 skills
curl http://localhost:7892/api/remote-skills-list
```
...
1. **检查 Agent 注册状态**：
```bash
curl -s http://127.0.0.1:7892/api/agents-status | python3 -m json.tool
```
...
```bash
# 手动触发巡检扫描（自动重试卡住的任务）
curl -X POST http://127.0.0.1:7892/api/scheduler-scan \
