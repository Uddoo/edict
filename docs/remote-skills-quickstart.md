...
# 输出: 三省六部看板启动 → http://127.0.0.1:7892
```
...
```bash
curl http://localhost:7892/api/remote-skills-list | jq .
...
```bash
curl -X POST http://localhost:7892/api/update-remote-skill \
  -H "Content-Type: application/json" \
  -d '{
    "agentId": "zhongshu",
    "skillName": "code_review"
...
```bash
curl -X POST http://localhost:7892/api/remove-remote-skill \
  -H "Content-Type: application/json" \
  -d '{
    "agentId": "zhongshu",
    "skillName": "code_review"
